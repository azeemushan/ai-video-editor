...
function ManageSubscription() {
  ...
  const handleDelete = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        setCurrentSubscription(null); // Reset currentSubscription state after deletion
      }
    } catch (error) {
    ...
    return (
      <div className="bg-white shadow-lg rounded-lg p-6 mb-4">
        <div className="mb-4 flex justify-between items-center border-b-2">
          <h3 className="text-lg font-semibold text-gray-900">{t('current-package')}</h3>
          <p className="text-sm text-gray-500">{subscriptionPackage.subscription_type}</p>
        </div>
        <div className="grid grid-cols-1 gap-4 mb-4">
          <div className="flex justify-between border-b-2">
            <span className="text-gray-700">{t('upload-limit')}</span>
            <span className="text-gray-900">{usage.upload_count}/{subscriptionPackage.upload_video_limit}</span>
          </div>
          <div className="flex justify-between border-b-2">
            <span className="text-gray-700">{t('generate-clips')}</span>
            <span className="text-gray-900">{usage.clip_count}/{subscriptionPackage.generate_clips}</span>
          </div>
          <div className="flex justify-between border-b-2">
            <span className="text-gray-700">{t('total-minutes')}</span>
            <span className="text-gray-900">{usage.min}/{subscriptionPackage.total_min}</span>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="container mx-auto pt-[50px] w-[45rem]">
      <div className="bg-white shadow-md rounded-lg p-6 mb-6">
        ...
        <div className="flex justify-end space-x-4">
          ...
          <Link href="/dashboard/manageSubscription/upgradeSubscription">
            <button className="pr-[8rem] pl-[8rem] bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded transition duration-300">
              {t('upgrade')}
            </button>
          </Link>
        </div>
      ...
      <ConfirmationModal
        ...
        onConfirm={handleDelete} // Pass handleDelete function to handle confirmation
      ...
      {showSuccess && (
        <div className="fixed top-4 left-1/2 transform -translate-x-1/2 z-50">
          <div className="bg-white text-red-800 text-lg font-semibold px-4 py-2 rounded-md shadow-lg">
            {t('delete-success-message')}
          </div>
        </div>
      )}
    </div>
